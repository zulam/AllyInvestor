...
#get list of top losers
urlLosers = 'https://api.tradeking.com/v1/market/toplists/toppctlosers.json?exchange=N'
auth = OAuth1Session(
...
try:
    losers = []
    keyList = ['symbol', 'pchg', 'last']

    r = auth.get(urlLosers)
    json_result = r.json()
    quotes = json_result['response']['quotes']
    losers = []
    for quote in quotes['quote']:
        losers.append({'symbol': quote['symbol'], 'pchg': quote['pchg'], 'last': quote['last']})
    for item in losers:
        print(item['symbol'] + ' - ' + item['pchg'] + ' - ' + item['last'] + '\n')
