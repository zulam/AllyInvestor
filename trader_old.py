...
def checkQuoteList(master_list):
    ...
    for ticker in master_list:
        ...
        r = auth.get(url)
        json_result = r.json()
        for quote in json_result['response']['quotes']:
            if quote['pchg'] >= 10:
                message = '\n' + 'Watch ' + ticker + ' at ' + str(quote['ask']) + ' (' \
                            + str(round(quote['pchg'], 4)) + ' gain since last close)'
...
def checkGains(tickerList):
    ...
    for quote in json_result['response']['quotes']['quote']:
        ...
        if percent_change >= 50:
            ...
            sendEmail(message)
            master_list.remove(sym)
            exclude.append(sym)

def checkHiLo(tickerList):
    url = 'https://api.tradeking.com/v1/market/ext/quotes.json?symbols='    
    ctr = 0
    for ticker in master_list:
        if ctr == 0:
